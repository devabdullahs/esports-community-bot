...
CREATE TABLE IF NOT EXISTS mvp_vote_nominees (
  id                BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  session_id        BIGINT NOT NULL REFERENCES mvp_vote_sessions(id) ON DELETE CASCADE,
...
ALTER TABLE ewc_profile_links ADD COLUMN IF NOT EXISTS public_identity_enabled INTEGER NOT NULL DEFAULT 1;
ALTER TABLE ewc_profile_links ADD COLUMN IF NOT EXISTS public_display_name TEXT;
ALTER TABLE ewc_profile_links ADD COLUMN IF NOT EXISTS public_avatar_url TEXT;
ALTER TABLE ewc_profile_links ADD COLUMN IF NOT EXISTS public_avatar_token TEXT;
CREATE UNIQUE INDEX IF NOT EXISTS idx_ewc_profile_links_public_avatar_token ON ewc_profile_links(public_avatar_token);
ALTER TABLE ewc_profile_links ADD COLUMN IF NOT EXISTS public_identity_updated_at TEXT;
...
ALTER TABLE ewc_news_posts ADD COLUMN IF NOT EXISTS scheduled_publish_at TEXT;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
      FROM pg_constraint
     WHERE conrelid = 'ewc_news_posts'::regclass
       AND contype = 'c'
    ...
    FOR constraint_name IN
      SELECT conname
       ...
       WHERE conrelid = 'ewc_news_posts'::regclass
         ...
         AND pg_get_constraintdef(oid) ILIKE '%status%'
    LOOP
      EXECUTE format('ALTER TABLE ewc_news_posts DROP CONSTRAINT %I', constraint_name);
    END LOOP;
    ALTER TABLE ewc_news_posts
      ADD CONSTRAINT ewc_news_posts_status_check
...
CREATE INDEX IF NOT EXISTS idx_ewc_news_posts_scheduled_publish
...
-- Community comments on durable targets (one-level threads; soft delete). See
-- the SQLite schema in src/db/index.js for the threading/visibility notes.
CREATE TABLE IF NOT EXISTS post_comments (
  id                BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  post_id           BIGINT REFERENCES ewc_news_posts(id) ON DELETE CASCADE,
...
ALTER TABLE post_comments ADD COLUMN IF NOT EXISTS target_type TEXT;
ALTER TABLE post_comments ADD COLUMN IF NOT EXISTS target_id BIGINT;
...
ALTER TABLE post_comments ALTER COLUMN target_type SET DEFAULT 'news';
ALTER TABLE post_comments ALTER COLUMN target_type SET NOT NULL;
ALTER TABLE post_comments ALTER COLUMN target_id SET NOT NULL;
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'post_comments_target_shape_check'
  ) THEN
    ALTER TABLE post_comments
      ADD CONSTRAINT post_comments_target_shape_check CHECK (
        (target_type = 'news' AND post_id = target_id)
        OR (target_type = 'match' AND post_id IS NULL)
      );
  END IF;
END $$;
CREATE INDEX IF NOT EXISTS idx_post_comments_target ON post_comments(target_type, target_id, status, created_at, id);
CREATE INDEX IF NOT EXISTS idx_post_comments_target_root ON post_comments(target_type, target_id, root_comment_id, status, created_at, id);
CREATE INDEX IF NOT EXISTS idx_post_comments_autoapprove ON post_comments(status, auto_approve_at);
CREATE INDEX IF NOT EXISTS idx_post_comments_moderation ON post_comments(status, created_at, id);

CREATE OR REPLACE FUNCTION delete_match_comments() RETURNS TRIGGER AS $$
BEGIN
  DELETE FROM post_comments WHERE target_type = 'match' AND target_id = OLD.id;
  RETURN OLD;
END;
$$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS delete_match_comments ON matches;
CREATE TRIGGER delete_match_comments
  AFTER DELETE ON matches
  FOR EACH ROW EXECUTE FUNCTION delete_match_comments();
...
CREATE TABLE IF NOT EXISTS web_product_events (
  ...
  path               TEXT NOT NULL,
  acquisition_source TEXT NOT NULL DEFAULT 'direct'
    CONSTRAINT web_product_analytics_acquisition_source_check
    CHECK (acquisition_source IN ('direct','x','discord','google','bing','other_referral')),
  campaign           TEXT
    CONSTRAINT web_product_analytics_campaign_check
    CHECK (campaign IS NULL OR campaign ~ '^[a-z0-9][a-z0-9_-]{0,63}$'),
  country            TEXT,
  occurred_at        BIGINT NOT NULL
);
...
-- BEGIN MIGRATION 0007-thg-survey.sql
