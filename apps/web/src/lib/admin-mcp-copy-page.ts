...
const ADMIN_MCP_COPY_PAGE_EN = [
  ...
  "Protocol revisions:",
  "",
  "- Revision `2026-07-28` (stateless). Every POST also carries `MCP-Protocol-Version: 2026-07-28`, `Mcp-Method`, and — for `tools/call` — `Mcp-Name`, with the same version repeated in `params._meta[\"io.modelcontextprotocol/protocolVersion\"]`. There is no `initialize` handshake and no session id; the bearer key is still required on every request.",
  "- Revisions `2025-11-25` and earlier, over the `initialize` handshake, unchanged.",
  "",
  "Send `server/discover` for the authoritative list. Because the tool set follows your key's grants, `tools/list` is returned with `cacheScope: \"private\"`.",
  "",
  "## Create A Key",
  "",
...
const ADMIN_MCP_COPY_PAGE_AR = [
  ...
  "",
  "إصدارات البروتوكول المدعومة:",
  "",
  "- الإصدار `2026-07-28` (بدون حالة). يحمل كل طلب POST أيضا `MCP-Protocol-Version: 2026-07-28` و`Mcp-Method`، ومع `tools/call` يحمل `Mcp-Name`، مع تكرار الإصدار نفسه في `params._meta[\"io.modelcontextprotocol/protocolVersion\"]`. لا توجد مصافحة `initialize` ولا معرف جلسة، ويبقى مفتاح Bearer مطلوبا في كل طلب.",
  "- الإصدارات `2025-11-25` وما قبلها عبر مصافحة `initialize` كما هي دون تغيير.",
  "",
  "أرسل `server/discover` للحصول على القائمة المعتمدة. وبما أن مجموعة الأدوات تتبع صلاحيات مفتاحك، تعاد `tools/list` مع `cacheScope: \"private\"`.",
  "",
  "## إنشاء مفتاح",
  "",
