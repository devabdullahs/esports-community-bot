import { MCP_TOOL_MANIFEST } from "@/lib/mcp-tool-manifest";
import type { Locale } from "@/lib/i18n";

function publicMcpToolLines(locale: Locale) {
  return MCP_TOOL_MANIFEST
    .filter((tool) => tool.surfaces.includes("public"))
    .map((tool) => `- \`${tool.name}\`: ${tool.description[locale]}`);
}

const PUBLIC_MCP_COPY_PAGE_EN = [
  "# Public MCP",
  "",
  "The public MCP endpoint lets AI assistants read the same public Esports Community data that appears on the website. No key is required.",
  "",
  "It is read-only and returns public website data intended for visitors.",
  "",
  "Docs page:",
  "",
  "```text",
  "https://esportscommunity.net/docs/mcp",
  "```",
  "",
  "## Endpoint",
  "",
  "Production:",
  "",
  "```text",
  "https://esportscommunity.net/api/public-mcp",
  "```",
  "",
  "Local development:",
  "",
  "```text",
  "http://localhost:3000/api/public-mcp",
  "```",
  "",
  "Required headers:",
  "",
  "```http",
  "Content-Type: application/json",
  "Accept: application/json, text/event-stream",
  "```",
  "",
  "No bearer key is required. JSON-RPC batch arrays are rejected; send one MCP request at a time.",
  "",
  "Protocol revisions:",
  "",
  "- Revision `2026-07-28` (stateless). Every POST also carries `MCP-Protocol-Version: 2026-07-28`, `Mcp-Method`, and — for `tools/call` — `Mcp-Name`, with the same version repeated in `params._meta[\"io.modelcontextprotocol/protocolVersion\"]`. There is no `initialize` handshake and no session id.",
  "- Revisions `2025-11-25` and earlier, over the `initialize` handshake, unchanged.",
  "",
  "Send `server/discover` for the authoritative list.",
  "",
  "## Tools",
  "",
  ...publicMcpToolLines("en"),
  "",
  "## Example questions",
  "",
  '- "Show the latest Valorant EWC news and include website links."',
  '- "Which clubs lead the EWC Club Championship standings?"',
  '- "Show the community EWC prediction leaderboard."',
  '- "Find Team Falcons and list its public profile details."',
  "",
  "Club Championship standings and community prediction rankings are separate datasets and use separate tools.",
  "",
  "## Client Setup",
  "",
  "Use native Streamable HTTP MCP configuration when your client supports it.",
  "",
  "For stdio-only clients, use mcp-remote:",
  "",
  "```json",
  "{",
  '  "mcpServers": {',
  '    "esports-community": {',
  '      "command": "npx",',
  '      "args": [',
  '        "-y",',
  '        "mcp-remote",',
  '        "https://esportscommunity.net/api/public-mcp"',
  "      ]",
  "    }",
  "  }",
  "}",
  "```",
  "",
  "For Codex config.toml stdio setup:",
  "",
  "```toml",
  "[mcp_servers.esports_community_public]",
  'command = "npx"',
  "args = [",
  '  "-y",',
  '  "mcp-remote",',
  '  "https://esportscommunity.net/api/public-mcp",',
  "]",
  "```",
  "",
  "For clients with native Streamable HTTP support:",
  "",
  "```toml",
  "[mcp_servers.esports_community_public]",
  "enabled = true",
  'url = "https://esportscommunity.net/api/public-mcp"',
  "```",
  "",
  "Restart the MCP client after changing configuration.",
  "",
  "## Security Notes",
  "",
  "- This endpoint is public and read-only.",
  "- Browser-origin requests are origin-checked.",
  "- Requests are rate-limited by trusted edge IP. If no trusted IP header is present, requests share one fallback bucket.",
  "- Responses are limited to public website fields.",
].join("\n");

const PUBLIC_MCP_COPY_PAGE_AR = [
  "# خادم MCP العام",
  "",
  "نقطة نهاية MCP العامة تتيح لأدوات الذكاء الاصطناعي قراءة نفس بيانات Esports Community العامة الظاهرة في الموقع. لا تحتاج إلى مفتاح.",
  "",
  "هذه النقطة للقراءة فقط وتعرض بيانات عامة مخصصة لزوار الموقع.",
  "",
  "صفحة الشرح:",
  "",
  "```text",
  "https://esportscommunity.net/ar/docs/mcp",
  "```",
  "",
  "## نقطة النهاية",
  "",
  "الإنتاج:",
  "",
  "```text",
  "https://esportscommunity.net/api/public-mcp",
  "```",
  "",
  "التطوير المحلي:",
  "",
  "```text",
  "http://localhost:3000/api/public-mcp",
  "```",
  "",
  "الترويسات المطلوبة:",
  "",
  "```http",
  "Content-Type: application/json",
  "Accept: application/json, text/event-stream",
  "```",
  "",
  "لا تحتاج إلى مفتاح Bearer. يتم رفض طلبات JSON-RPC batch؛ أرسل طلب MCP واحدا في كل مرة.",
  "",
  "إصدارات البروتوكول المدعومة:",
  "",
  "- الإصدار `2026-07-28` (بدون حالة). يحمل كل طلب POST أيضا `MCP-Protocol-Version: 2026-07-28` و`Mcp-Method`، ومع `tools/call` يحمل `Mcp-Name`، مع تكرار الإصدار نفسه في `params._meta[\"io.modelcontextprotocol/protocolVersion\"]`. لا توجد مصافحة `initialize` ولا معرف جلسة.",
  "- الإصدارات `2025-11-25` وما قبلها عبر مصافحة `initialize` كما هي دون تغيير.",
  "",
  "أرسل `server/discover` للحصول على القائمة المعتمدة.",
  "",
  "## الأدوات",
  "",
  ...publicMcpToolLines("ar"),
  "",
  "## \u0623\u0645\u062b\u0644\u0629 \u0639\u0644\u0649 \u0627\u0644\u0623\u0633\u0626\u0644\u0629",
  "",
  '- "\u0627\u0639\u0631\u0636 \u0622\u062e\u0631 \u0623\u062e\u0628\u0627\u0631 Valorant \u0641\u064a EWC \u0645\u0639 \u0631\u0648\u0627\u0628\u0637 \u0627\u0644\u0645\u0648\u0642\u0639."',
  '- "\u0645\u0627 \u0627\u0644\u0623\u0646\u062f\u064a\u0629 \u0627\u0644\u0645\u062a\u0635\u062f\u0631\u0629 \u0641\u064a \u062a\u0631\u062a\u064a\u0628 \u0628\u0637\u0648\u0644\u0629 \u0623\u0646\u062f\u064a\u0629 EWC\u061f"',
  '- "\u0627\u0639\u0631\u0636 \u0644\u0648\u062d\u0629 \u062a\u0631\u062a\u064a\u0628 \u062a\u0648\u0642\u0639\u0627\u062a \u0645\u062c\u062a\u0645\u0639 EWC."',
  '- "\u0627\u0628\u062d\u062b \u0639\u0646 Team Falcons \u0648\u0627\u0639\u0631\u0636 \u0628\u064a\u0627\u0646\u0627\u062a \u0645\u0644\u0641\u0647 \u0627\u0644\u0639\u0627\u0645."',
  "",
  "\u062a\u0631\u062a\u064a\u0628 \u0628\u0637\u0648\u0644\u0629 \u0627\u0644\u0623\u0646\u062f\u064a\u0629 \u0648\u062a\u0631\u062a\u064a\u0628 \u062a\u0648\u0642\u0639\u0627\u062a \u0627\u0644\u0645\u062c\u062a\u0645\u0639 \u0645\u062c\u0645\u0648\u0639\u062a\u0627 \u0628\u064a\u0627\u0646\u0627\u062a \u0645\u0646\u0641\u0635\u0644\u062a\u0627\u0646 \u0648\u0644\u0643\u0644 \u0645\u0646\u0647\u0645\u0627 \u0623\u062f\u0627\u062a\u0647\u0627.",
  "",
  "## إعداد العميل",
  "",
  "استخدم إعداد Streamable HTTP MCP الأصلي عندما يدعمه العميل.",
  "",
  "للعملاء الذين يدعمون stdio فقط، استخدم mcp-remote:",
  "",
  "```json",
  "{",
  '  "mcpServers": {',
  '    "esports-community": {',
  '      "command": "npx",',
  '      "args": [',
  '        "-y",',
  '        "mcp-remote",',
  '        "https://esportscommunity.net/api/public-mcp"',
  "      ]",
  "    }",
  "  }",
  "}",
  "```",
  "",
  "لإعداد Codex عبر config.toml باستخدام stdio:",
  "",
  "```toml",
  "[mcp_servers.esports_community_public]",
  'command = "npx"',
  "args = [",
  '  "-y",',
  '  "mcp-remote",',
  '  "https://esportscommunity.net/api/public-mcp",',
  "]",
  "```",
  "",
  "للعملاء الذين يدعمون Streamable HTTP مباشرة:",
  "",
  "```toml",
  "[mcp_servers.esports_community_public]",
  "enabled = true",
  'url = "https://esportscommunity.net/api/public-mcp"',
  "```",
  "",
  "أعد تشغيل عميل MCP بعد تغيير الإعدادات.",
  "",
  "## ملاحظات الأمان",
  "",
  "- هذه النقطة عامة وللقراءة فقط.",
  "- طلبات المتصفح يتم التحقق من Origin لها.",
  "- الطلبات محددة المعدل حسب عنوان IP الموثوق من الطرف. عند عدم وجود ترويسة IP موثوقة، تشترك الطلبات في حاوية معدل واحدة.",
  "- تقتصر الاستجابات على حقول الموقع العامة.",
].join("\n");

export const PUBLIC_MCP_COPY_PAGE_BY_LOCALE: Record<Locale, string> = {
  en: PUBLIC_MCP_COPY_PAGE_EN,
  ar: PUBLIC_MCP_COPY_PAGE_AR,
};

export const PUBLIC_MCP_COPY_PAGE = PUBLIC_MCP_COPY_PAGE_EN;

export function getPublicMcpCopyPage(locale: Locale) {
  return PUBLIC_MCP_COPY_PAGE_BY_LOCALE[locale];
}
