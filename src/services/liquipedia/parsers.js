...
// Stream hosts our live poller can actually track. YouTube/Soop are intentionally
// excluded: the poller only checks Twitch + Kick, so importing those would add
// channels that can never light up the per-match "live" strip.
const STREAM_HOSTS = {
  'twitch.tv': 'twitch',
...
// First path segments that are site sections, not channels.
const NON_CHANNEL_SEGMENTS = new Set([
  'videos', 'video', 'directory', 'p', 'team', 'teams', 'clips', 'clip',
  'about', 'live', 'settings', 'subscriptions', 'following', 'downloads',
  'jobs', 'turbo', 'prime', 'store', 'search', 'category', 'categories',
]);

// Parse a Twitch/Kick channel URL to { platform, handle } (handle lowercased), or
// null if it isn't a plain channel link (VOD/clip/section/other host).
export function streamChannelFromUrl(href) {
  let url;
  try {
  ...
  const platform = STREAM_HOSTS[url.hostname.toLowerCase()];
  if (!platform) return null;
  const segments = url.pathname.split('/').filter(Boolean);
  if (!segments.length) return null;
  const handle = segments[0].toLowerCase();
  if (NON_CHANNEL_SEGMENTS.has(handle)) return null;
  // /<channel>/clip/…, /<channel>/video/…, /<channel>/v/… are VOD/clip links.
  if (segments[1] && /^(clip|clips|video|videos|v)$/i.test(segments[1])) return null;
  // Twitch/Kick handles: letters, digits, underscore (Twitch) — keep it tight so a
  // stray path doesn't become a bogus channel.
  if (!/^[a-z0-9_]{2,30}$/.test(handle)) return null;
  return { platform, handle };
}

// The section heading (mw-headline text) a top-level block sits under. MediaWiki
// output is a flat list of headings + content blocks beneath .mw-parser-output, so
// we climb to the block that is a direct child of that root, then read the nearest
// preceding heading. Used to skip the "Broadcast Talent" table (casters' personal
// links) while keeping the "Streams" table.
function sectionHeadingOf($, el) {
  const root = $(el).closest('.mw-parser-output');
  let block = $(el);
  while (block.parent().length && !block.parent().is(root) && !block.parent().is('body')) {
    block = block.parent();
  }
  const heading = block.prevAll('h2, h3, h4').first();
  return (heading.find('.mw-headline').text() || heading.text()).trim();
}

// Official broadcast stream channels for a tournament: the Twitch/Kick channels
// Liquipedia lists in the infobox icon row and the "Streams" table (per language /
// partner). Deliberately skips the "Broadcast Talent" table (casters' personal
// channels) and any VOD/clip links. Returns deduped [{ platform, handle }].
export function parseBroadcasterStreams($) {
  const found = new Map(); // `${platform}:${handle}` -> { platform, handle }
  const add = (href) => {
    const channel = streamChannelFromUrl(href);
    if (channel) found.set(`${channel.platform}:${channel.handle}`, channel);
  };

  // 1) Infobox icon row — the primary official channel(s). Non-stream links (home,
  //    social) are filtered out by streamChannelFromUrl's host check.
  $('.infobox-icons a[href]').each((_, a) => add($(a).attr('href')));

  // 2) Stream tables, excluding the Broadcast Talent section.
  $('table.wikitable').each((_, table) => {
    if (/broadcast talent|talent|casters?/i.test(sectionHeadingOf($, table))) return;
    $(table)
      .find('a[href]')
      .each((_, a) => add($(a).attr('href')));
  });
